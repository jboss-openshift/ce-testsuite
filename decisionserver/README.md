...
This testsuite will test alll Decision Server S2I application templates which are:
...
###### Optional ce-arq parameteres
  - -Dtest=**The test class name, if you want to run only one test, otherwise all test will be executed**
...
```sh
$ mvn clean install -Pdecisionserver -Dkubernetes.master=https://192.168.1.254:8443 -Dkubernetes.registry.url=192.168.1.254:5001 -Ddocker.url=http://192.168.1.254:2375 -Drouter.hostIP=192.168.1.254
```
###### Running a specific test and ignoring the cleanup after the tests gets finished
Example:
```sh
$ mvn clean install -Pdecisionserver -Dkubernetes.master=https://192.168.1.254:8443 -Dkubernetes.registry.url=192.168.1.254:5001 -Ddocker.url=http://192.168.1.254:2375 -Drouter.hostIP=192.168.1.254 -Dtest=DecisionServerAmqTest -Dkubernetes.ignore.cleanup=true
```
...
#### Found an issue?
Please, feel free to report the issue [here](https://github.com/jboss-openshift/ce-testsuite/issues/new).
