...
@ExternalDeployment
@Template(url = "https://raw.githubusercontent.com/bdecoste/application-templates/ssoTP/sso/sso70-postgresql.json",
		labels = "application=ssopostgresql,component=server",
        parameters = {
                @TemplateParameter(name = "APPLICATION_NAME", value="ssopostgresql"),
                @TemplateParameter(name = "HOSTNAME_HTTP", value="ssopostgresql${openshift.domain}"),
                @TemplateParameter(name = "HOSTNAME_HTTPS", value="secure-ssopostgresql${openshift.domain}"),
...
public class SsoPostgresqlTest extends SsoTestBase
{
	...
	public SsoPostgresqlTest() {
		route = "ssopostgresql";
		secureRoute = "secure-ssopostgresql";
	}
}
